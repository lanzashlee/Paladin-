...
function SectionHeader({ eyebrow, title, description, dark = false }) {
  return (
    <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
      ...
      <h2
        className={`mt-5 text-3xl sm:text-4xl md:text-5xl font-black tracking-tight ${dark ? 'text-white' : 'text-[#012E72]'}`}
        style={{ fontFamily: FONT_DISPLAY }}
      >
        {title}
      </h2>
      <p
        className={`mt-4 text-sm sm:text-base md:text-lg leading-relaxed text-justify ${dark ? 'text-[#F7F4EF]' : 'text-[#010407]/75'}`}
        style={{ fontFamily: FONT_BODY }}
      >
        {description}
      </p>
    ...
    <article className={`relative flex h-full flex-col overflow-hidden rounded-3xl border border-[#e7dccb] bg-white group shadow-lg shadow-[#012E72]/8 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#012E72]/12 ${className}`}>
      <div className="relative h-44 sm:h-48 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 h-12 min-w-12 px-2 rounded-2xl bg-white border border-[#d8cbb8] flex items-center justify-center shrink-0 shadow-md">
          <span className="text-[#012E72] text-xs sm:text-sm font-black leading-none text-center" style={{ fontFamily: FONT_DISPLAY }}>{item.badge}</span>
        </div>
      </div>

      <div className="relative z-10 p-5 sm:p-6 flex flex-1 flex-col">
        <div className="flex items-center justify-end gap-4 mb-2">
          <span className="text-[11px] uppercase tracking-[0.22em] text-[#002DB5] font-semibold" style={{ fontFamily: FONT_BODY }}>Insurance Line</span>
        </div>

        <h3
          className="text-lg sm:text-xl font-bold text-[#012E72] mb-3 leading-tight group-hover:text-[#002DB5] transition-colors"
          style={{ fontFamily: FONT_SUBHEADING, fontStyle: 'italic' }}
        >
          {item.title}
        </h3>
        <p className="text-[#010407]/75 text-sm sm:text-[15px] leading-relaxed text-justify" style={{ fontFamily: FONT_BODY }}>
          {item.description}
        </p>
      </div>
    </article>
