    ...
    <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
      <p
        className={`inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold tracking-[0.22em] uppercase border ${
          dark
            ? 'bg-white/5 text-[#7dd3fc] border-white/10'
            : 'bg-[#0077b6]/10 text-[#0077b6] border-[#0077b6]/15'
        }`}
      >
        {eyebrow}
      </p>
      <h2 className={`mt-5 text-3xl sm:text-4xl md:text-5xl font-black tracking-tight ${dark ? 'text-white' : 'text-[#0d1117]'}`}>
        {title}
      </h2>
      <p className={`mt-4 text-sm sm:text-base md:text-lg leading-relaxed ${dark ? 'text-white/70' : 'text-gray-600'}`}>
        {description}
      </p>
    </div>
  );
...
function InsuranceLineCard({ item }) {
  return (
    <article className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 min-h-[280px] group shadow-2xl shadow-black/10">
      <div
      ...
      <div className="relative z-10 p-5 sm:p-6 flex h-full flex-col">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="w-12 h-12 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/10 flex items-center justify-center shrink-0">
            <span className="text-white text-sm font-black">{item.badge}</span>
          </div>
          <span className="text-[11px] uppercase tracking-[0.22em] text-white/60 font-semibold">Insurance Line</span>
        </div>

        <h3 className="text-lg sm:text-xl font-bold text-white mb-2 group-hover:text-[#7dd3fc] transition-colors">
          {item.title}
        </h3>
        ...
        <ul className="mt-4 space-y-2 text-xs sm:text-sm text-white/85">
          {item.details.map((detail) => (
            <li key={detail} className="flex gap-3">
              <span className="mt-1 h-2 w-2 rounded-full bg-[#7dd3fc] shrink-0" aria-hidden="true" />
              <span>{detail}</span>
            </li>
          ))}
        </ul>
...
function CoverageCard({ item }) {
  return (
    <article className="relative overflow-hidden rounded-3xl border border-white/20 p-5 sm:p-6 flex flex-col gap-3 group min-h-[280px] hover:-translate-y-1.5 hover:shadow-xl hover:shadow-blue-500/15 transition-all duration-300">
      <div
      ...
      <div className="relative z-10 flex items-center justify-between">
        <div className="w-10 h-10 rounded-2xl bg-white/15 backdrop-blur-sm border border-white/20 flex items-center justify-center shadow-md shadow-blue-500/20 shrink-0">
          <span className="text-white text-sm font-black">{item.number}</span>
        </div>
        <span className="text-[11px] uppercase tracking-[0.2em] text-white/80 font-semibold">Featured</span>
      </div>

      <h3 className="relative z-10 text-lg sm:text-xl font-bold text-white drop-shadow-sm group-hover:text-[#7dd3fc] transition-colors">
        {item.title}
      </h3>
      <p className="relative z-10 text-white/95 text-xs sm:text-sm leading-relaxed text-justify drop-shadow-sm flex-1">
        {item.description}
      </p>

      <ul className="relative z-10 mt-1 space-y-2 text-xs sm:text-sm text-white/95">
        {item.details.map((detail) => (
          <li key={detail} className="flex gap-3">
            <span className="mt-1 h-2 w-2 rounded-full bg-[#7dd3fc] shrink-0" aria-hidden="true" />
            <span>{detail}</span>
...
function Service() {
  return (
    <>
      ...
      <main className="bg-[#ecf3f8]">
        <section className="relative bg-[#0a0a0a] py-20 md:py-24 overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[300px] bg-[#0077b6]/10 rounded-full blur-3xl" />
          </div>
