...
const reasons = [
  {
    icon: <Handshake className="w-8 h-8 text-[#002DB5]" />,
  ...
  {
    icon: <CircleDollarSign className="w-8 h-8 text-[#002DB5]" />,
  ...
  {
    icon: <Star className="w-8 h-8 text-[#002DB5]" />,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 pb-8">
          {reasons.map((r, i) => (
            <div key={i} className="bg-white/10 backdrop-blur-md border border-white/15 rounded-3xl p-8 hover:bg-white/15 transition-colors shadow-lg shadow-[#010407]/10">
              ...
              <p className="text-[#F7F4EF] leading-relaxed text-sm md:text-base text-justify">{r.desc}</p>
            </div>
          ))}
        </div>
      </div>
